...
function resolveAssetPath(filename) {
  ...
  const parent = path.join(rootDir, "..", filename);
...
function getLiveReactions(store) {
  if (!store.liveReactions || typeof store.liveReactions !== "object") {
    store.liveReactions = { likes: 0, dislikes: 0, users: {} };
  }
  if (!store.liveReactions.users || typeof store.liveReactions.users !== "object") {
    store.liveReactions.users = {};
  }
  store.liveReactions.likes = Number(store.liveReactions.likes || 0);
  store.liveReactions.dislikes = Number(store.liveReactions.dislikes || 0);
...
async function sendDailyReminders() {
  ...
  for (const capsule of pending) {
    ...
    const result = await sendEmail({
      to: capsule.email,
      attachments: bannerAttachment ? [bannerAttachment] : [],
      subject: `DearFutureMe reminder — ${daysLabel} left`,
      text: [
        `Hello, ${capsule.name}.`,
        "",
        `There are ${daysLabel} left until your capsule opens.`,
        `Open date: ${openDate}.`,
        "",
        `Open the site: ${appBaseUrl}`
      ].join("\n"),
      html: `
        <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;color:#201810">
          ${getEmailBannerHtml({ useCid: shouldUseInlineImages() })}
          <h2 style="margin:0 0 16px">DearFutureMe reminder</h2>
          <p>Hello, <strong>${escapeHtml(capsule.name)}</strong>.</p>
          <p>There are <strong>${escapeHtml(daysLabel)}</strong> left until your capsule opens.</p>
          <p>Open date: <strong>${escapeHtml(openDate)}</strong>.</p>
          <p style="margin-top:24px"><a href="${escapeAttribute(appBaseUrl)}">Open DearFutureMe</a></p>
        </div>
      `
    });
...
app.post("/api/live/reaction", authRequired, async (req, res) => {
  ...
  res.json({ likes, dislikes, state: next });
});
...
app.get("/seal-sound.mp3", (_req, res) => {
  res.sendFile(resolveAssetPath("_ui_window_close_menu_screen_03_normal.mp3"));
});

app.get("/hammer-sound.mp3", (_req, res) => {
  res.sendFile(resolveAssetPath("0be1c1022f8ed63.mp3"));
});

app.get("/windy-sound-effects.mp3", (_req, res) => {
  res.sendFile(resolveAssetPath("windy-sound-effects.mp3"));
});
